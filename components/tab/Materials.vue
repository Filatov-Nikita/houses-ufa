<template>
  <div>
    <div class="tw-overflow-auto tw-mb-6 xl:tw-mb-8">
      <BaseTabsGroup v-model="tab" class="">
        <BaseTabsGroupItem v-for="item in teamTabs" :name="item.name">
          {{ item.title }}
        </BaseTabsGroupItem>
      </BaseTabsGroup>
    </div>
    <BaseTabsTabContent v-model="tab">
      <BaseTabsTabContentItem name="roof" key="roof">
        <div
          class="tw-rounded-2xl tw-overflow-hidden tw-h-[412px] lg:tw-h-[560px]"
        >
          <Swiper
            :modules="[SwiperPagination, SwiperNavigation]"
            :slides-per-view="1"
            :pagination="{
              type: 'bullets',
              el: '.swiper-pagination',
              dynamicBullets: true,
            }"
            :navigation="{
              nextEl: '.swiper-next',
              prevEl: '.swiper-prev',
            }"
            class="tw-h-full lg:tw-w-full"
          >
            <SwiperSlide class="tw-relative tw-h-full tw-w-full">
              <img
                src="/assets/images/img/country_real_estate.png"
                alt=""
                class="tw-object-cover tw-h-full tw-absolute tw-w-full"
              />
              <div
                class="tw-absolute tw-left-6 tw-bottom-6 tw-hidden lg:tw-grid lg:tw-gap-4 lg:tw-w-[507px] tw-bg-white tw-rounded-2xl tw-p-6"
              >
                <h4 class="tw-text-h5">Крыша</h4>
                <p class="tw-text-body_m">
                  Фальцевая кровля домов выполнена из оцинкованного
                  металлопрофиля
                </p>
                <p class="tw-text-text02 tw-text-body_xs">
                  Визуализация проекта предварительная, возможны изменения
                </p>
              </div>
              <button
                class="tw-absolute tw-grid tw-place-content-center lg:tw-hidden tw-top-4 tw-right-4 tw-w-12 tw-h-12 tw-rounded-lg tw-text-white tw-bg-white/50"
                @click="
                  openPopup(
                    'Крыша',
                    'Фальцевая кровля домов выполнена из оцинкованного металлопрофиля'
                  )
                "
              >
                <BaseIcon class="tw-w-6 tw-h-6" name="info" />
              </button>
            </SwiperSlide>
            <SwiperSlide class="tw-relative tw-h-full tw-w-full">
              <img
                src="/assets/images/img/country_real_estate.png"
                alt=""
                class="tw-object-cover tw-h-full tw-absolute tw-w-full"
              />
              <div
                class="tw-absolute tw-left-6 tw-bottom-6 tw-hidden lg:tw-grid lg:tw-gap-4 lg:tw-w-[507px] tw-bg-white tw-rounded-2xl tw-p-6"
              >
                <h4 class="tw-text-h5">Крыша</h4>
                <p class="tw-text-body_m">
                  Фальцевая кровля домов выполнена из оцинкованного
                  металлопрофиля
                </p>
                <p class="tw-text-text02 tw-text-body_xs">
                  Визуализация проекта предварительная, возможны изменения
                </p>
              </div>
            </SwiperSlide>
            <button
              class="swiper-next tw-w-8 tw-h-8 tw-rounded-lg tw-backdrop-blur-sm tw-bg-text01/40 tw-grid tw-place-content-center tw-absolute tw-top-1/2 -tw-translate-y-1/2 tw-right-6 tw-z-10"
            >
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M17.0306 12.531L9.53055 20.031C9.46087 20.1007 9.37815 20.156 9.2871 20.1937C9.19606 20.2314 9.09847 20.2508 8.99993 20.2508C8.90138 20.2508 8.8038 20.2314 8.71276 20.1937C8.62171 20.156 8.53899 20.1007 8.4693 20.031C8.39962 19.9614 8.34435 19.8786 8.30663 19.7876C8.26892 19.6965 8.24951 19.599 8.24951 19.5004C8.24951 19.4019 8.26892 19.3043 8.30663 19.2132C8.34435 19.1222 8.39962 19.0395 8.4693 18.9698L15.4396 12.0004L8.4693 5.03104C8.32857 4.89031 8.24951 4.69944 8.24951 4.50042C8.24951 4.30139 8.32857 4.11052 8.4693 3.96979C8.61003 3.82906 8.80091 3.75 8.99993 3.75C9.19895 3.75 9.38982 3.82906 9.53055 3.96979L17.0306 11.4698C17.1003 11.5394 17.1556 11.6222 17.1933 11.7132C17.2311 11.8043 17.2505 11.9019 17.2505 12.0004C17.2505 12.099 17.2311 12.1966 17.1933 12.2876C17.1556 12.3787 17.1003 12.4614 17.0306 12.531Z"
                  fill="white"
                />
              </svg>
            </button>
            <button
              class="swiper-prev tw-w-8 tw-h-8 tw-rounded-lg tw-backdrop-blur-sm tw-bg-text01/40 tw-grid tw-place-content-center tw-absolute tw-top-1/2 -tw-translate-y-1/2 tw-left-6 tw-z-10"
            >
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M15.5307 18.9698C15.6004 19.0395 15.6557 19.1222 15.6934 19.2132C15.7311 19.3043 15.7505 19.4019 15.7505 19.5004C15.7505 19.599 15.7311 19.6965 15.6934 19.7876C15.6557 19.8786 15.6004 19.9614 15.5307 20.031C15.461 20.1007 15.3783 20.156 15.2873 20.1937C15.1962 20.2314 15.0986 20.2508 15.0001 20.2508C14.9016 20.2508 14.804 20.2314 14.7129 20.1937C14.6219 20.156 14.5392 20.1007 14.4695 20.031L6.96948 12.531C6.89974 12.4614 6.84443 12.3787 6.80668 12.2876C6.76894 12.1966 6.74951 12.099 6.74951 12.0004C6.74951 11.9019 6.76894 11.8043 6.80668 11.7132C6.84443 11.6222 6.89974 11.5394 6.96948 11.4698L14.4695 3.96979C14.6102 3.82906 14.8011 3.75 15.0001 3.75C15.1991 3.75 15.39 3.82906 15.5307 3.96979C15.6715 4.11052 15.7505 4.30139 15.7505 4.50042C15.7505 4.69944 15.6715 4.89031 15.5307 5.03104L8.56041 12.0004L15.5307 18.9698Z"
                  fill="white"
                />
              </svg>
            </button>
            <div class="swiper-pagination !tw-left-auto !-tw-right-4"></div>
          </Swiper>
        </div>
      </BaseTabsTabContentItem>
    </BaseTabsTabContent>
  </div>
  <BaseModal v-model="popup" v-slot="{ hide }" :forMob="true">
    <BaseModalCard v-bind="{ hide }">
      <div class="tw-grid">
        <h5 class="tw-text-h6 tw-mb-4">
          {{ popupMessage.title }}
        </h5>
        <p class="tw-text-body_s2 -tw-tracking-875 tw-mb-4">
          {{ popupMessage.text }}
        </p>
        <p class="tw-text-text02 tw-text-body_xs tw-mb-6">
          Визуализация проекта предварительная, возможны изменения
        </p>
        <BaseButton theme="gray" @click="hide"> Закрыть </BaseButton>
      </div>
    </BaseModalCard>
  </BaseModal>
</template>
<script lang="ts" setup>
const tab = ref('roof')
const teamTabs = [
  {
    name: 'roof',
    title: 'Крыша',
  },
  {
    name: 'foundation',
    title: 'Фундамент',
  },
  {
    name: 'walls',
    title: 'Стены',
  },
  {
    name: 'windows',
    title: 'Окна',
  },
]
const popup = ref(false)
const popupMessage = ref({
  title: '',
  text: '',
})
const openPopup = (title: string, text: string) => {
  popupMessage.value = {
    title,
    text,
  }
  popup.value = true
}
</script>
<style lang="scss" scoped></style>
