...
const filtredLocations = computed(() => props.locations)
...
function initMap() {
  map = new ymaps.Map(props.id, {
    zoom: 13,
    center: getLatLng(props.objectData),
    controls: [],
  })
  ...
  const radius = new ymaps.GeoObject(
    {
      geometry: {
        type: 'Circle',
        coordinates: getLatLng(props.objectData),
        radius: 1000,
      },
    },
    {
...
function appendComplex(map: any): void {
  const placemark = new ymaps.Placemark(
    getLatLng(props.objectData),
    {
      hintContent: data.name,
      balloonContentHeader: data.name,
      imgSrc: props.objectData.sight_picture?.url,
    },
    {
...
function getLatLng<T extends { longitude: string; latitude: string }>(
  location: T
): [number, number] {
