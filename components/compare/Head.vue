<template>
  <div class="">
    <div class="tw-grid tw-content-start tw-gap-6">
      <div
        class="tw-rounded-2xl tw-overflow-hidden tw-relative tw-min-h-[147px] tw-min-w-[147px] tw-w-full tw-h-full tw-bg-secondary tw-justify-self-center"
      >
        <button
          class="tw-rounded-lg tw-w-8 tw-h-8 tw-bg-white tw-absolute tw-right-4 tw-top-4 tw-z-10 tw-grid tw-place-content-center tw-p-2.5"
        ...
        <img
          ...
          alt=""
          class="tw-object-contain tw-absolute tw-top-1/2 tw-left-1/2 tw-transform -tw-translate-x-1/2 -tw-translate-y-1/2"
        />
      </div>
      <div class="tw-grid tw-gap-2">
        <h5
          v-if="description?.typeHome === 'cottage'"
          class="tw-text-text02 tw-text-body_s2"
        >
          {{ description.title }}
        </h5>
        ...
        <p
          v-if="description?.typeHome === 'apartment'"
          class="tw-text-text01 tw-text-body_xs"
        >
          {{ description.title }} м²
        </p>
      </div>
...
const compareHeadProps = defineProps<{
  description: decriptionHeadApartment
}>()
</script>
