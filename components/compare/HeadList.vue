<template>
  <div class="products-slider tw-relative" ref="compare">
    ...
    <button
      class="tw-w-10 tw-h-10 tw-bg-secondary tw-rounded tw-grid tw-place-content-center prev tw-absolute tw-top-1/2 tw-transform -tw-translate-y-1/2 -tw-translate-x-full -tw-left-6"
      @click="prev"
    >
      <img
    ...
    <button
      class="tw-w-10 tw-h-10 tw-bg-secondary tw-rounded tw-grid tw-place-content-center next tw-absolute tw-top-1/2 tw-transform -tw-translate-y-1/2 tw-translate-x-full -tw-right-6"
      @click="next"
    >
      <img
        src="/assets/images/icons/arrow_left.svg"
  ...
  <div class="products-slider__fixed mx-[88px]" v-if="isVisibleCompareAbsolute">
...
const next = () => {
  nameAnimation.value = 'fade'
  if (currentIndex.value < values.value.length - 4) currentIndex.value += 1
}
onMounted(() => {
  windowData.value = headValues.value.slice(0, 4)
})
...
.products-slider__fixed {
  ...
  @apply tw-rounded-2xl tw-py-4 tw-px-[68px] tw-bg-white tw-shadow-shadow00;
  &-wrapper {
    @apply tw-flex tw-flex-wrap tw-overflow-hidden tw-gap-5 tw-justify-between;
    & > div {
      width: calc(25% - 1.5rem);
