<template>
  <div class="compare__body">
      <CompareItemGroup
        v-if="generalValuesKeys.room_factor"
        title="Комнатность"
        :data-list="generalValuesKeys.room_factor"
        :border-bottom="true"
      >
        <template #item="{ item }">
          {{ $getRoomsCount(item) }}
      ...
      <CompareItemGroup
        v-if="generalValuesKeys.area_total"
        title="Площадь"
        :data-list="generalValuesKeys.area_total"
        :border-bottom="true"
      >
        <template #item="{ item }">
          {{ $formatValue(item, 'м²') }}
      ...
      <CompareItemGroup
        v-if="generalValuesKeys.mortgage_monthly_payment"
        title="В ипотеку"
        :data-list="generalValuesKeys.mortgage_monthly_payment"
        :border-bottom="true"
      >
        <template #item="{ item }">
          {{ $amount(item) }}
    ...
    <div class="accordion tw-mt-6">
      ...
      <div class="accordion__body" :class="{ active: openLocation }">
        <div class="accordion__body-content">
          <CompareItemGroup
            v-if="generalValuesKeys['complex.location']"
            title="Локация"
            :data-list="generalValuesKeys['complex.location']"
            :border-bottom="true"
          />
          <CompareItemGroup
            v-if="generalValuesKeys['house.letter']"
            title="Литер"
            :data-list="generalValuesKeys['house.letter']"
            :border-bottom="true"
          />
          <CompareItemGroup
            v-if="generalValuesKeys.floor_number"
            title="Этаж"
            :data-list="generalValuesKeys.floor_number"
            :border-bottom="true"
          />
        </div>
      </div>
